  ...
  for (int i = 0; i < ObfTimes; i++) {
    for (GlobalVariable *GV : GVs) {
      ...
      if (GV->hasInitializer() && GV->getInitializer() &&
        ...
        auto *intData = dyn_cast<ConstantInt>(initializer);
        auto *arrData = dyn_cast<ConstantDataArray>(initializer);
        if (arrData) {
...
void GlobalsEncryption::insertArrayDecryption(Module &M, EncryptedGV encGV) {
  ...
  auto *arrTy = cast<ArrayType>(encGV.GV->getValueType());
