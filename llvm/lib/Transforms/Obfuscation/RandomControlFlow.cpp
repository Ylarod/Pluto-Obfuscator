...
// 混淆次数，混淆次数越多混淆结果越复杂
static cl::opt<int>
    ObfTimes("rcf-times", cl::init(1),
              cl::desc("Run RandomControlFlow pass <rcf-times> time(s)"));
...
void RandomControlFlow::insertRandomBranch(Value *randVar, BasicBlock *ifTrue,
                                           BasicBlock *ifFalse,
                                           BasicBlock *insertAfter) {
  // 对随机数进行等价变换
  Value *alteredRandVar = alterVal(randVar, insertAfter);
  Value *randMod2 = BinaryOperator::Create(Instruction::And, alteredRandVar,
                                           CONST_I32(1), "", insertAfter);
  auto *condition =
      new ICmpInst(*insertAfter, ICmpInst::ICMP_EQ, randMod2, CONST_I32(1));
...
bool RandomControlFlow::randcf(BasicBlock *BB) {
  ...
  BasicBlock *bodyBB =
      entryBB->splitBasicBlock(BB->getFirstNonPHIOrDbgOrLifetime(), "bodyBB");
  BasicBlock *endBB = bodyBB->splitBasicBlock(bodyBB->getTerminator(), "endBB");
  BasicBlock *cloneBB = createCloneBasicBlock(bodyBB);

  // 在 entryBB 后插入随机跳转，使其能够随机跳转到第 bodyBB 或其克隆基本块
  // cloneBB
  entryBB->getTerminator()->eraseFromParent();
  // 老方案：通过rdrand指令生成随机数
  // Function *rdrand = Intrinsic::getDeclaration(entryBB->getModule(),
  // Intrinsic::x86_rdrand_32); CallInst *randVarStruct =
  // CallInst::Create(rdrand->getFunctionType(), rdrand, "", entryBB);
  // // 通过 rdrand 内置函数获取随机数
  // Value *randVar = ExtractValueInst::Create(randVarStruct, 0, "", entryBB);

  // 方案一：通过分配一块栈空间实现类似于获取随机数的效果
  auto *randPtr = new AllocaInst(TYPE_I32, 0, "rand.ptr", entryBB);
  Value *randVar = new LoadInst(TYPE_I32, randPtr, "", entryBB);

  // 方案二：调用rand()函数获取随机数
  // Module &M = *BB->getModule();
  // Function *randFunc = cast<Function>(M.getOrInsertFunction("rand",
  // FunctionType::getInt32Ty(*CONTEXT)).getCallee());
  // randFunc->setDSOLocal(true);
  // Value *randVar = CallInst::Create(randFunc->getFunctionType(), randFunc,
  // "", entryBB);
...
std::random_device RandomControlFlow::rd;