...
// 混淆次数，混淆次数越多混淆结果越复杂
static cl::opt<int>
    ObfTimes("vsb-times", cl::init(1),
             cl::desc("Run VariableSubstitution pass <gle-times> time(s)"));
...
void VariableSubstitution::linearSubstitute(Instruction *I, unsigned int i) {
  Module &M = *I->getModule();
  auto *val = cast<ConstantInt>(I->getOperand(i));
  ...
  auto *constC = (ConstantInt *)CONST(type, c);
  // 创建全局变量 x, y
  auto *x = new GlobalVariable(M, type, false, GlobalValue::PrivateLinkage,
                               constX, "x");
  auto *y = new GlobalVariable(M, type, false, GlobalValue::PrivateLinkage,
                               constY, "y");
  auto *opX = new LoadInst(type, x, "", I);
  auto *opY = new LoadInst(type, y, "", I);
  // 构造 op = ax + by + c 表达式
...
void VariableSubstitution::bitwiseSubstitute(Instruction *I, unsigned int i) {
  Module &M = *I->getModule();
  auto *val = cast<ConstantInt>(I->getOperand(i));
  IntegerType *type = val->getType();
...
void VariableSubstitution::substitute(Instruction *I) {
  unsigned int operandNum = I->getNumOperands();
  for (unsigned int i = 0; i < operandNum; i++) {
