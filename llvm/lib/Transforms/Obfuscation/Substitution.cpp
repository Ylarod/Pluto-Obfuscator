...
void Substitution::substituteAdd(BinaryOperator *BI) const {
  unsigned int choice = cryptoutils->get_uint32_t() % NUMBER_ADD_SUBST;
  switch (choice) {
  case 0:
    addNeg(BI);
    break;
...
void Substitution::addNeg(BinaryOperator *BI) const {
  Value *op;
...
void Substitution::addDoubleNeg(BinaryOperator *BI) const {
  Value *op, *op1, *op2;
...
void Substitution::addRand(BinaryOperator *BI) const {
  auto *r = (ConstantInt *)CONST(BI->getType(), cryptoutils->get_uint32_t());
  Value *op;
...
void Substitution::addRand2(BinaryOperator *BI) const {
  auto *r = (ConstantInt *)CONST(BI->getType(), cryptoutils->get_uint32_t());
  Value *op;
...
void Substitution::substituteSub(BinaryOperator *BI) const {
  unsigned int choice = cryptoutils->get_uint32_t() % NUMBER_SUB_SUBST;
  switch (choice) {
  case 0:
    subNeg(BI);
    break;
...
void Substitution::subNeg(BinaryOperator *BI) const {
  Value *op;
  op = builder->CreateNeg(BI->getOperand(1));
...
void Substitution::subRand(BinaryOperator *BI) const {
  auto *r = (ConstantInt *)CONST(BI->getType(), cryptoutils->get_uint32_t());
  Value *op;
  op = builder->CreateAdd(BI->getOperand(0), r);
...
void Substitution::subRand2(BinaryOperator *BI) const {
  auto *r = (ConstantInt *)CONST(BI->getType(), cryptoutils->get_uint32_t());
  Value *op;
  op = builder->CreateSub(BI->getOperand(0), r);
  op = builder->CreateSub(op, BI->getOperand(1));
...
void Substitution::substituteXor(BinaryOperator *BI) const {
  unsigned int choice = cryptoutils->get_uint32_t() % NUMBER_XOR_SUBST;
  switch (choice) {
  case 0:
    xorSubstitute(BI);
    break;
...
void Substitution::xorSubstitute(BinaryOperator *BI) const {
  Value *op, *op1, *op2;
...
void Substitution::xorSubstituteRand(BinaryOperator *BI) const {
  auto *r = (ConstantInt *)CONST(BI->getType(), cryptoutils->get_uint32_t());
...
void Substitution::substituteAnd(BinaryOperator *BI) const {
  unsigned int choice = cryptoutils->get_uint32_t() % NUMBER_AND_SUBST;
  switch (choice) {
  case 0:
    andSubstitute(BI);
    break;
...
void Substitution::andSubstitute(BinaryOperator *BI) const {
  Value *op;
...
void Substitution::andSubstituteRand(BinaryOperator *BI) const {
  auto *r = (ConstantInt *)CONST(BI->getType(), cryptoutils->get_uint32_t());
  Value *op, *op1;
...
void Substitution::substituteOr(BinaryOperator *BI) const {
  unsigned int choice = cryptoutils->get_uint32_t() % NUMBER_OR_SUBST;
  switch (choice) {
  case 0:
    orSubstitute(BI);
    break;
...
void Substitution::orSubstitute(BinaryOperator *BI) const {
  Value *op, *op1;
...
void Substitution::orSubstituteRand(BinaryOperator *BI) const {
  auto *r = (ConstantInt *)CONST(BI->getType(), cryptoutils->get_uint32_t());
  Value *op, *op1;
  op = builder->CreateNot(BI->getOperand(0));
  op1 = builder->CreateNot(BI->getOperand(1));
