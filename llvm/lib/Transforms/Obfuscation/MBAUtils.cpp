...
int64_t *llvm::generateLinearMBA(unsigned int exprNumber) {
  auto *exprSelector = new unsigned int[exprNumber];
  auto *coeffs = new int64_t[15];
  while (true) {
    std::fill_n(coeffs, 15, 0);
    for (unsigned int i = 0; i < exprNumber; i++) {
    ...
    bool reject = false;
    for (unsigned int i = 0; i < exprNumber; i++) {
      coeffs[exprSelector[i]] += (int64_t)X[i];
...
Value *llvm::insertLinearMBA(int64_t *params, Instruction *insertBefore) {
  ...
  if (isa<BinaryOperator>(insertBefore) &&
    ...
    auto *xPtr = new GlobalVariable(M, type, false, GlobalValue::PrivateLinkage,
                                    CONST(type, randX), "x");
    auto *yPtr = new GlobalVariable(M, type, false, GlobalValue::PrivateLinkage,
                                    CONST(type, randY), "y");
