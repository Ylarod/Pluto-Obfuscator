...
static cl::opt<int>
    ObfTimes("trap-angr-times", cl::init(1),
             cl::desc("Run the TrapAngr pass <trap-angr-times> time(s)"));

static cl::opt<int> ObfProb("trap-angr-prob", cl::init(40),
                            cl::desc("Each constant will be substituted with "
                                     "a probability of <trap-angr-prob>%"));
      ...
      for (BasicBlock &BB : F) {
        ...
        for (Instruction *I : origInst) {
          if (isa<StoreInst>(I) || isa<CmpInst>(I) || isa<BinaryOperator>(I)) {
            ...
            for (unsigned int j = 0; j < operandNum; j++) {
              if (isa<ConstantInt>(I->getOperand(j)) &&
                  cryptoutils->get_uint8_t() % 100 < ObfProb) {
                substitute(I, j);
              }
            }
          }
